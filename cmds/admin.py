...
class Admin(Cog_Extension):
    ...
    async def on_member_join(self, member):
        ...
        if member.guild.id == int(jdata['rabbit_guild']['guild_id']):
            # print(f'{member} 加入了牛牛神殿')
            # channel = member.guild.get_channel(int(jdata['welcome_channel']))
            role = member.guild.get_role(
                int(jdata['rabbit_guild']['guest_role']))
            tw = pytz.timezone('Asia/Taipei')
            await member.add_roles(role)

            embed = discord.Embed(
                title=" ",
                description=f'歡迎來到{member.guild}~\n記得先看完 <#743353331363217418>\n之後再按照 <#746312391955841074> '
                            f'的指示來驗證並正式加入本群\n',
                color=0xf5ed00, timestamp=datetime.datetime.now(tz=tw))
            embed.set_author(
                name="牛牛の僕", icon_url="https://imgur.com/za5ATTg.jpg")
    ...
    async def on_member_remove(self, member):
        ...
        with open('members.json', 'r', encoding='utf8') as jfile:
            jdata = json.load(jfile)

        if member.id in jdata["member_id"]:
            if jdata[f'{member.id}']["custom_role"]:
                role = member.guild.get_role(
                    jdata[f'{member.id}']["custom_role"])
                await role.delete(reason="成員退出")
            jdata["member_id"].remove(member.id)
            del jdata[f'{member.id}']

            with open('members.json', 'w', encoding='utf8') as jfile:
    ...
    async def on_message(self, msg):
        ...
        # 驗證系統
        if msg.content == '同意' and msg.author != self.bot.user and msg.channel.id == int(
                jdata['rabbit_guild']['auth_channel']):
            await msg.delete()
            guest = msg.guild.get_role(
                int(jdata["rabbit_guild"]['guest_role']))
            await msg.author.remove_roles(guest)

            tw = pytz.timezone('Asia/Taipei')
            embed = discord.Embed(title=" ",
                                  description=f'恭喜你正式加入本群:)\n記得去 <#743854911078531219> 領取身分組\n(這是自動息請勿回覆,如有問題請直接私訊管理員)',
                                  color=0xf5ed00, timestamp=datetime.datetime.now(tz=tw))
            embed.set_author(
                name="牛牛の僕", icon_url="https://imgur.com/za5ATTg.jpg")
            await msg.author.send(embed=embed)

        elif msg.content != '同意' and msg.author != self.bot.user and msg.channel.id == int(
                jdata["rabbit_guild"]["auth_channel"]):
            await msg.delete()
            await msg.channel.send(f'{msg.author.mention} 驗證失敗，請再看仔細一點 ༼ ◕д ◕ ༽', delete_after=10)

        # 歡迎訊息
        if msg.channel.id == int(jdata['rabbit_guild']['welcome_channel']) and msg.author.id == 276060004262477825:
            tw = pytz.timezone('Asia/Taipei')
            embed = discord.Embed(
                title=" ",
                description=f'☆歡迎 {newer.mention} 來到Rabbit♡Fairy ☆\n\n新仁先去 <#743353331363217418> 了解我們的規定\n再來通過 '
                            f'<#746312391955841074> 即可成為我們的一員\n這邊 <#743854911078531219> 可以選擇你常玩的遊戲\n以及你自己喜歡的顏色('
                            f'ฅ´ω`ฅ)\n\n☆♡☆♡☆♡☆♡☆♡☆♡☆♡',
                color=0xf5ed00, timestamp=datetime.datetime.now(tz=tw))
            embed.set_author(
                name="牛牛の僕", icon_url="https://imgur.com/za5ATTg.jpg")
        ...
        # 簽到系統
        if msg.channel.id == 753543338006806528 and msg.content == "簽" and not msg.author.bot:
            with open("members.json", "r", encoding="utf8") as jfile:
                jdata = json.load(jfile)

            if msg.author.id in jdata["member_id"]:
                if jdata[f'{msg.author.id}']["today"]:
                    await msg.channel.send(f'{msg.author.mention} 你今天已經簽到了!')
                    return
                if msg.author.name != jdata[f'{msg.author.id}']["name"]:
                    jdata[f'{msg.author.id}']["name"] = msg.author.name
                if msg.author.display_name != jdata[f'{msg.author.id}']["nickname"]:
                    jdata[f'{msg.author.id}']["nickname"] = msg.author.display_name

                jdata[f'{msg.author.id}']["today"] = True
                jdata[f'{msg.author.id}']["total"] += 1
            else:
                jdata[f'{msg.author.id}'] = {"name": msg.author.name, "nickname": msg.author.display_name,
                                             "total": 1, "today": True, "custom_role": False, "remain": False}
                jdata["member_id"].append(msg.author.id)

            with open("members.json", "w", encoding="utf8") as jfile:
                json.dump(jdata, jfile, indent=4)

            await msg.channel.send(f'{msg.author.mention} 簽到成功!，這是你連續簽到的第 **{jdata[str(msg.author.id)]["total"]}** 天')
    ...
    async def apply(self, ctx, name: str, color: discord.Colour):
        """申請自訂身分組。用法：/申請 名稱 色碼"""

        with open('settings.json', 'r', encoding='utf8') as jfile:
            jdata = json.load(jfile)

        if ctx.channel.id != 753542568939356220 and ctx.author.id not in jdata["owner"]:
        ...
        if ctx.author.id in jdata["member_id"]:
            if jdata[f'{ctx.author.id}']["total"] >= 3 or jdata[f'{ctx.author.id}']["remain"]:
                if not jdata[f'{ctx.author.id}']["custom_role"]:
                    ...
                    jdata[f'{ctx.author.id}']["custom_role"] = role.id
                    with open('members.json', 'w', encoding='utf8') as jfile:
                        json.dump(jdata, jfile, indent=4)

                    await ctx.author.add_roles(role, reason="連續簽到3天獎勵")
                    await ctx.send(f'{ctx.author.mention} 申請自訂身分組成功')
                elif jdata[f'{ctx.author.id}']["custom_role"]:
                    role = ctx.guild.get_role(
                        jdata[f'{ctx.author.id}']["custom_role"])
